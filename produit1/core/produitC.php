...
function afficherProduit1 ($produits){
	...
	function modifierProduit($produits){
		$sql="UPDATE produits SET pho=:pho,code=:code,designation=:designation,ttva=:ttva,qte=:qte,prix_unit=:prix_unit,remis=:remis,couleur=:couleur WHERE code=:code";
...
try{
		...
		$datas = array(':pho'=>$pho,':code'=>$code,':designation'=>$designation,':ttva'=>$ttva,':qte'=>$qte,':prix_unit'=>$prix_unit,':remis'=>$remis,':couleur'=>$couleur);
		$req->bindValue(':pho',$pho);
		$req->bindValue(':code',$code);
		$req->bindValue(':designation',$designation);
		$req->bindValue(':ttva',$ttva);
